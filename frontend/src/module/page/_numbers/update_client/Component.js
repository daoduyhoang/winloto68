...
import { Row , Col, Button, Menu, Dropdown, Input, InputNumber, Icon, Radio, Notification, Table } from 'antd'
...
export default class extends StandardPage {
    ...
    renderClients() {
        ...
        return (
            <Col span={24} >
                <Col span={24} >
                    {this.state.selectedClientName ? this.state.selectedClientName : "Danh sách"} 
                    {!this.state.showClient &&
                        <Button type="button" className="marginLeft" onClick={() => this.showClients()}>danh sách <Icon type="down" /></Button>
                    }
                    {this.state.selectedClientName && this.state.showClient &&
                        <Button type="button" className="marginLeft" onClick={() => this.hideClients()}>ẩn <Icon type="up" /></Button>
                    }
                </Col>
                <Col span={24} >
                    {
                        this.state.showClient && clients.map(client => {
                            return (<Button type="button" onClick={() => this.onSelectClient(client)}> {client.name} </Button>)
                        })
                    }
                </Col>
            </Col>
        ...
        return (
            <div className="normalText">
                ...
                <Col span={12}>
                    ...
                    <Col span={24}><p>Hồi</p></Col>
                    <Col span={12}>
                        <Col span={24} >
                            <Col span={8}>Hồi 2 con(%)</Col>
                            <Col span={14}>                            
                                <div className="input-bet">                        
                                <InputNumber
                                style={{ width: '100%'}}
                ...
                <Col span={24}>
                    ...
                    <Col span={12}>
                        <Button className="maxWidth" onClick={() => this.updateClient()}> lưu </Button>
                    </Col>
                    <Col span={12}>
                        <Button className="maxWidth" onClick={() => this.resetClient()}> quay lại </Button>
                    </Col>
                </Col>
            </div>
        )
    ...
    ord_renderContent() {
        return (
                <div>
                <Col xs={0} md={5}></Col>
                <Col offset={1} xs={22} md={12}>
                    <Col span={24}  className="normalText"><p>Sửa thông tin khách</p> </Col>
                    <Col className="normalText" span={24} >
                    {this.renderClients()}
                    {this.state.selectedClient && this.renderAddClient()}
                    </Col>
                </Col>
                </div>
        );
