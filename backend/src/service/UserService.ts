...
const selectFields = '-salt -password -elaBudget -elaOwed -votePower -resetToken -hashKey'
...
export default class extends Base {
    ...
    public async resetPKey() {
        ...
        await db_user.update({_id: this.currentUser._id}, doc)
        return account.privateKey
    }

    public async resetPKeyByAdmin(param) {
        const msg = param.msg
        ...
        const user = await db_user.findOne({username: param.username})
        if (!user) {
        ...
        await db_user.update({_id: user._id}, doc)
        return account.privateKey
    }

    private async msg_exist(msg) {
        const db_message = this.getDBModel('Message');
        const message = await db_message.findOne({text: msg})
    ...
    public async autoCreateAccount(param) {
        ...
        let invitedBy = null
        if (param.invitedBy) {
            let _invitedBy = (param.invitedBy).toLowerCase()
            invitedBy = await db_user.findOne({username: _invitedBy})
            if (!invitedBy) {
                throw 'invitedBy not found'
            }
        }
        ...
        if (invitedBy) doc['invitedBy'] = invitedBy._id
    ...
    public async recordLogin(param) {
        ...
        const user = await db_user.findOne({ _id: param.userId })
    ...
    public async getUserByPKey(privateKey) {
        const db_user = this.getDBModel('User');
        const user = await db_user.db.findOne({privateKey: privateKey});
    ...
    public async getUserByPW(param) {
        const db_user = this.getDBModel('User');
        const username = param.username
        const user = await db_user.db.findOne({username: username});
        const password = this.getPassword(param.password, user.salt)
    ...
    ************************************************************************************
    ...
    public async changePassword(param){
        ...
        const res = await db_user.update({_id: user._id}, {
    ...
    * */
    ...
    public validate_username(username){
        if(!validate.valid_string(username, 4)){
